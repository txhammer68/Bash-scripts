...
EXTIP=`curl -s ipecho.net/plain` >/dev/null
INTIP=`ip -4 addr show eno1 | grep -oP '(?<=inet\s)\d+(\.\d+){3}'` >/dev/null
echo "{\"hostname\":\"$HOSTNAME\",\"extip\":\"$EXTIP\",\"intip\":\"$INTIP\",\"distro\":\"$DISTRO\",\"code\":\"$CODE\",\"firewall\":\"$FIREWALL\",\"release\":\"$RELEASE\",\"kernel\":\"$KERNEL\",\"sda\":\"$SDATEST\",\"sdb\":\"$SDBTEST\",\"sdc\":\"$SDCTEST\"}" > /home/Data/osInfo.json
